...
const app = createApp();
const testEmail = `bookings-list-test-${Date.now()}@example.com`;
...
beforeAll(async () => {
  const admin = await prisma.admin.create({
    data: { email: testEmail, passwordHash: await hashPassword(testPassword), name: 'Bookings List Test Admin' },
  });
  adminId = admin.id;
