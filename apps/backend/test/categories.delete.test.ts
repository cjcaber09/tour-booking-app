...
const app = createApp();
const testEmail = `categories-delete-test-${Date.now()}@example.com`;
...
beforeAll(async () => {
  const admin = await prisma.admin.create({
    data: { email: testEmail, passwordHash: await hashPassword(testPassword), name: 'Categories Delete Test Admin' },
  });
  adminId = admin.id;
