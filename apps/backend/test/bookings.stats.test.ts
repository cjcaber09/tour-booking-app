...
const app = createApp();
const testEmail = `bookings-stats-test-${Date.now()}@example.com`;
...
beforeAll(async () => {
  const admin = await prisma.admin.create({
    data: { email: testEmail, passwordHash: await hashPassword(testPassword), name: 'Bookings Stats Test Admin' },
  });
  adminId = admin.id;
